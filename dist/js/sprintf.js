...
function toInteger(n, f, w, p) {
  ...
  if (-1 < n && n < 1) str = "0";
    ...
    if (pos_e != -1) {
      ...
      var pos_dot = str.indexOf(".");
      if (pos_dot != -1) str = str.substring(0, pos_dot);
    }
  }
  ...
  if (pos_e != -1) {
    ...
    if (exp > 0) {
      ...
      if (equive_p < -1)
        ...
        var pos_dot = str.indexOf(".");
        if (pos_dot != -1) str = str.charAt(0) + str.substring(2, pos_e);
        str = "000000" + str;
        for (exp += 7; exp; exp++) str = "0" + str;
        str = "0." + str;
      } else {
        //微妙
    ...
    if (dec > p) {
      ...
      if (tmp > 5) {
        ...
        if (n < 1) {
          ...
          if (tmp.length > p + 2 - i) {
            //桁増
            if (i == 2) str = "1." + tmp.substring(1);
            else str = str.substring(0, i - 1) + tmp;
          } else str = str.substring(0, i) + tmp;
        } else {
          ...
          else str = tmp.substring(0, pos_dot) + "." + tmp.substring(pos_dot);
        }
      } else {
        //切捨て
      ...
      //"0"を充填
      for (var i = p - dec; i; i--) str += "0";
    }
  } else {
    if (p) {
      str += ".0";
      for (var i = p - 1; i; i--) str += "0";
    }
  }
...
function toExponential(n, f, w, p, e) {
  ...
  if (type == 0) {
    //整数
    if ((exp = str.length - 1)) str = str.charAt(0) + "." + str.substring((pos_dot = 1));
  } else if (type == 1) {
    //小数
    if (n > 10) {
      exp = pos_dot - 1;
  ...
  //指数部の整形
  if (exp >= 0) str += e + (exp < 10 ? "+0" : "+") + exp;
  else str += e + (exp > -10 ? "-0" + -exp : exp);
...
function ProcFlag(str, f, extra, b) {
  ...
  if ((f & 4) > 0 && !minus) {
    ...
    if (extra > 0) {
      var c = "0";
      for (var i = extra; i; i--) str = c + str;
    }
    if (!b) str = "-" + str;
    else if (plus) str = "+" + str;
  } else {
    //without 0 or with -
    if (!b) str = "-" + str;
    else if (plus) str = "+" + str;
    ...
    if (extra > 0) {
      var c = " ";
      if (minus) for (var i = extra; i; i--) str += c;
      else for (var i = extra; i; i--) str = c + str;
    }
  }
  if (space) str = " " + str;
